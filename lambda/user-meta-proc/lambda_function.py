...
def processObject(objectKey, s3BucketName, s3BucketArn):
    # fetch meta from preload table
    preloadMetaTable = dynamoDbResource.Table(FssiResources.DynamoDB.MediaUserMetaPreload)
    res = preloadMetaTable.get_item(Key={'id' : objectKey})
    if 'Item' in res:
        print('retrieved',res['Item'])
        # save it to final table
        userMeta = makeMediaMetaItem(objectKey, res['Item']['bucket'])
        userMeta['meta'] = res['Item']['meta']
        userMetaTable = dynamoDbResource.Table(FssiResources.DynamoDB.MediaUserMeta)
        userMetaTable.put_item(Item = userMeta)
        # delete record from preload table
        preloadMetaTable.delete_item(Key={'id': objectKey})
    else:
        raise ValueError('couldn\'t retireve meta from preload table for {}'.format(objectKey))
